  ...
  const groupedOrders = useMemo(() => {
    ...
    localOrders.forEach((order) => {
      if (order.scheduledFor) {
        const dateKey = formatDateShort(order.scheduledFor);
    ...
    <div className="rounded-xl border border-[#ead9cd] bg-white p-3 sm:p-6 dark:border-[#4a3c30] dark:bg-[#2a1e14]">
      ...
      {hasOrders ? (
        <div className="space-y-8">
          {groupedOrders.map(({ date, orders, colorClass }) => (
            <div key={date} className="space-y-4">
              {/* Header com data */}
              <div className={`${colorClass} text-white rounded-lg px-6 py-4 shadow-lg`}>
                <div className="flex items-center gap-3">
                  <Calendar className="h-6 w-6" />
                  <div>
                    <h3 className="text-xl font-bold">
                      {formatDate(orders[0].scheduledFor!)}
                    </h3>
                    <p className="text-sm opacity-90">
                      {orders.length} pedido{orders.length > 1 ? 's' : ''} agendado{orders.length > 1 ? 's' : ''}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 @container md:grid-cols-2 xl:grid-cols-3">
                {orders.map((order) => (
                  <article
                    key={order.id}
                    className="flex h-full flex-col gap-4 rounded-lg border-2 border-[#ead9cd] bg-white p-4 shadow-sm transition hover:shadow-lg dark:border-[#4a3c30] dark:bg-[#23170f]"
                  >
                    ...
                    {/* Horário agendado - DESTAQUE */}
                    {order.scheduledFor && (
                      <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border-l-4 border-green-500 rounded-lg p-3">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-xs text-[#a16b45]">
                          Criado: {formatTime(order.createdAt)}
                        </p>
                      </div>
                    </div>
                          ...
                          <li key={item.id} className="flex items-start justify-between gap-3">
                            <div className="flex-1">
                              ...
                              {item.selectedOptions && (
                                <p className="text-xs text-[#a16b45] line-clamp-1">
                      ...
                      <div className="rounded-md bg-[#fdf9f5] p-3 text-xs text-[#a16b45] dark:bg-[#1d140c] dark:text-[#dfc7b4]">
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            <p className="line-clamp-2">{formatAddress(order.deliveryAddress)}</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="mt-auto flex flex-col gap-2 sm:flex-row">
                      ...
                      <Button
                        ...
                        variant="outline"
                        disabled={isPending || pendingAction === `print:${order.id}`}
                        size="sm"
                      >
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <Calendar className="mb-4 h-16 w-16 text-[#dfc7b4]" />
          <h3 className="mb-2 text-lg font-semibold text-[#333333] dark:text-[#f5f1e9]">
            Nenhum pedido agendado
          </h3>
          <p className="text-sm text-[#a16b45]">
            Os pedidos agendados aparecerão aqui organizados por data.
          </p>
        </div>
      )}
    </div>
